...
public class SistemaDeGestaoDePedidos {
    ...
    private static void pagarPedido(int id, double valorPago) {
        ...
        if (!pedidos.containsKey(id))
            ...
            if (pedido.getValor() == valorPago && !pedido.isPago()) {
                pedido.setPago(true);
    ...
    private static void cancelarPedido(int id) {
        ...
        if (pedidos.containsKey(id)) {
            Pedido pedido = pedidos.get(id);
            if (pedido.isPago())
                logger.warn("Não é possível cancelar o pedido ID {} pois já foi pago.", 1);
            else {
                pedidos.remove(id);
                logger.info("Cancelamento do pedido ID {} realizado com sucesso!", id);
            }
        } else
